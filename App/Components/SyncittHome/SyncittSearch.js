...
import { updateScene } from "../MaintenanceRequestComponent/MaintenanceAction";
...
class SyncittSearch extends Component {
    ...
    componentDidMount() {
        ...
        if(navigation){
            if (this.props.navigation.getParam('fromTraderProfile')) {
                ...
                this.setState({ renderSlide: 2, trader_id: _id})
                if (userData != null) {
                    let address = this.state.loggedUserInfo.data ?this.state.loggedUserInfo.data.address  : "";
                    this.setState({
    ...
    onRequestSubmit() {
        // console.log("this.state", this.state.loggedUserInfo)
        if (this.state.reqSubject == "") {
            ...
            if(userData === '' || userData === null){
                ...
                let requestParams = {
                    "budget": this.state.reqBudget !== '' ? Number(this.state.reqBudget):0,
                    "category_id":  this.state.newCategoryId,
                    ...
                    // "created_by": userData?userData.data._id:0,
                ...
                console.log('paramsdatadataa1',requestParams);
                ...
                var requestParams = {
                    ...
                    "longitude": this.state.reqLongitude ? Number(this.state.reqLongitude) : (this.state.selectedLocation && this.state.selectedLocation.geometry && this.state.selectedLocation.geometry.location && this.state.selectedLocation.geometry.location.lng),
                    "dt": this.state.reqDate,
                    created_by_role: this.state.mainRole?this.state.mainRole:0,
                    request_type: 0,
                ...
                console.log('Paramsdatadataa',requestParams);
                APICaller('addMR', 'post', "", requestParams).then(data => {
                    if (data.code == 200) {
                        ...
                        if (this.state.documentsList.length > 0) {
                            this.state.documentsList.forEach(element => {
                                ...
                                body.append("_id", data.data._id);
                                console.log(body, "bodybody")
                                documentUpload("uploadMaintenanceImages","",body,"").then(
                                    data => {
                                        if (data.code == 200) {
                        ...
                        var objMaintenanceUsers = {
                            id: userData.data._id,
                            maintenanceId: MRid,
                            firstName: userData.data.firstname,
                            lastName: userData.data.lastname
                        }
                        console.log('SocketaddMaintenanceUsers',objMaintenanceUsers);
                        this.socket.emit('addMaintenanceUsers', objMaintenanceUsers);
                        var objMessage = {
                        ...
                        console.log('SocketmaintenanceGroupMessageSent',objMessage);
    ...
    onLocationChangeText(val, reqAddress) {
        ...
        GetLocation(val).then(data => {
            ...
            if (data.predictions.length > 0) {
                console.log(JSON.stringify(data.predictions) + "  Resppppponse data.predictions")
    ...
    onPressLocation(element, reqAddress) {
       // alert(JSON.stringify(element))
        this.setState({ locationSearchText: element.description, predictions: [] })
        if (reqAddress == "reqAddress") {
            // alert(JSON.stringify(element))
        ...
        if (this.state.searchText == '' || JSON.stringify(this.state.selectedItem) == '{}') {
            ...
            if (this.state.selectionType == 'request') {
                this.setState({
                    ...
                    trader_id: '',
                    showMobileNumber: false,
                    newCategoryId : this.state.selectedItem._id,
                ...
                if (this.state.loggedUserInfo != null) {
                    ...
                    page_number = 1;
                    await this.setState({traderload:true,searchTradersList:[]});
                    this._getTradeListAPICalled(page_number);
    ...
    _getTradeListAPICalled = (page_number) => {
        let postcode = '';
        let suburb = '';
        let state = '';
        ...
        let category = (this.state.selectedItem.name).toLowerCase().split(' ').join('_');
        category = category.split('_-_').join('_');
        let requestParams = {
        ...
        if(this.state.traderload){
            this.setState({ loading: true })
            console.log('page_number',requestParams);
            APICaller('tradersList', 'POST', "", requestParams).then(data => {
                if (data.code == 200) {
                    this.setState({ loading: false })
                    if (data.data.length > 0) {
                        console.log(`Page Number ${page_number}`,data.data);
                        let listData = this.state.searchTradersList;
                        let traderData = listData.concat(data.data); 
                        if(this.state.traderload){
                            this.setState({
                                showSearchLoader: false, 
                                renderSlide: 3,
    ...
    navigateToTrader(item) {
        this.setState({traderload:false})
            ...
            .then(value => {
                ...
                if (JSON.parse(value)) {
                    Actions.MessageToTraderScreen({
                        ...
                        receiverId: this.state.traderDetails._id, goBackToPop: true
                    });
                }
                else {
                    ...
                    <ImageBackground source={IMAGEPATH.BACKGROUND_IMAGE} style={{ flex: 1 }}>
                        ...
                        <View style={style.transparentFormContainer}>
                            ...
                            <View style={style.formHeaderTextContainer}>
                                ...
                                {this.props.navigation&&this.props.navigation.getParam('fromTraderProfile') ?
                                    ...
                                    <View style={style.inputBoxContainer2}>
                                        <DatePicker
                                            ...
                                            onDateChange={(date) => {
                                                ...
                                                this.setState({ reqDate: new Date(date) });
                                            }}
                                        />
                    ...
                    <View style={style.whiteBackgroundContainer}>
                        ...
                        <View style={{ alignItems: 'center', marginBottom: Matrics.ScaleValue(15), borderWidth: this.state.predictions.length > 0 ? 1 : 0, borderColor: '#eee', backgroundColor: 'white', justifyContent: 'center', borderRadius: Matrics.ScaleValue(5), alignSelf: 'center', width: '100%' }}>
                            {this.state.predictions.map(element => {
                                return (
                                    <TouchableOpacity onPress={this.onPressLocation.bind(this, element, "reqAddress")} style={{ height: Matrics.ScaleValue(45), alignSelf: 'center', alignItems: 'center', borderBottomWidth: 0.5, borderBottomColor: 'lightgray', width: '100%', flexDirection: 'row', paddingLeft: Matrics.ScaleValue(15) }}>
                                        <View style={{ flex: 0.1 }}>
    ...
    renderIntialHomeSearch() {
        return (
            <KeyboardAvoidingView style={style.renderIntialSearchContainer} behavior={Platform.OS == 'android' ? '' : 'padding'} enabled>
                <ScrollView nestedScrollEnabled={true} keyboardShouldPersistTaps={'handled'} style={style.renderIntialSearchContainer}>
                    ...
                    <View style={style.headerTitleStyle}>
                        <Text style={style.headerTextStyle}>Get Tradie Quotes <Text style={{ fontWeight: 'bold' }}>Fast</Text></Text>
                    </View>
        ...
        return (
            <View style={style.renderTraderContainerStyle}>
                <View style={style.logoContainer}>
                    {item.data.image ?
                        <ImageBackground imageStyle={{ borderRadius: Matrics.ScaleValue(60) }} source={{ uri: API.USER_IMAGE_PATH + item.data.image }} style={style.profileImageStyle}>
                            {item.data.is_online &&
                        ...
                        <View style={[style.profileImageStyle, { backgroundColor: COLORS.BACKGROUND, justifyContent: 'center', alignItems: 'center' }]}>
                            <Text style={{ color: 'white', fontSize: Matrics.ScaleValue(25), fontWeight: '600' }}>{(item.data.business_name && item.data.business_name.charAt(0).toUpperCase())}</Text>
                        </View>
                    }
                </View>
                <View style={style.profileDataContainer}>
                    <Text style={style.traderNameTextStyle}>{item.data.business_name ? item.data.business_name :item.data.firstname+ ' ' +item.data.lastname}</Text>
                    <View>
                        <StarRating
                            disabled={true}
                            maxStars={5}
                            ...
                            rating={item.data.averageRate}
                            selectedStar={rating => ref.onStarRatingPress(rating)}
                        />
                    </View>
                    <Text style={style.reviewsStyle}>
                        {item.data.totalReviewLength ? '(' + item.data.totalReviewLength + ')' + 'Reviews'
                            :
                            'No Reviews'}
                    </Text>
                    <View style={style.servicesContainer}>
                        <FlatList
                            data={item.data.categories_id && item.data.categories_id}
                            extraData={this.state}
                    ...
                    <View style={style.footerButtonContainer}>
                        ...
                        <TouchableOpacity onPress={() => {
                            this.setState({ renderSlide: 2, trader_id: item._id,newCategoryId:this.state.selectedItem._id, selectedCategory: this.state.selectedItem, reqCategory: this.state.selectedItem.name })
                            if (this.state.loggedUserInfo != null) {
                                let address = this.state.loggedUserInfo.data ?this.state.loggedUserInfo.data.address  : "";
                                this.setState({
                                    reqName: this.state.loggedUserInfo.data.name,
            ...
            <Modal onRequestClose={() => this.setState({ renderSlide: 1 })} transparent={true} animationType={'slide'} visible={this.state.renderSlide == 3}>
                <ImageBackground source={IMAGEPATH.BACKGROUND_IMAGE} style={style.renderTraderContainer}>
                    ...
                    <View style={style.traderTransparentContainer}>
                        <TouchableOpacity style={style.backIcon} onPress={async()=> await this.setState({renderSlide:1,traderload:false,searchTradersList: []})}>
                        <Image resizeMode='contain' style={{alignSelf:'center',height:'100%',width:'100%'}} source={IMAGEPATH.BACK_WHITE} />
    ...
    handleLoadMore = () => {
        if (!this.state.loading && this.state.traderload) {
            page_number = page_number + 1;
            this._getTradeListAPICalled(page_number); // method for API call 
        }
    };
    _traderprofilebackpress = async() => {
        page_number = 1;
        await this.setState({traderload:true,searchTradersList:[]});
        await this._getTradeListAPICalled(page_number); // method for API call 
                    ...
                    <ScrollView>
                        {bannerImage ?
                            <ImageBackground source={{ uri: API.USER_IMAGE_PATH + bannerImage }} style={{ height: Matrics.ScaleValue(180), backgroundColor: '#0073E6', width: '100%' }}>
                                <View>
                                    <TouchableOpacity onPress={() =>this._traderprofilebackpress() } style={{ backgroundColor: COLORS.SKY_BLUE_BUTTON_BACKGROUND, width: Matrics.ScaleValue(40), height: Matrics.ScaleValue(40), padding: 10, margin: Matrics.ScaleValue(30), marginTop: Matrics.ScaleValue(50), borderRadius: 20, alignItems: 'center', justifyContent: 'center' }}>
                                        <Image source={IMAGEPATH.BACK_WHITE} resizeMode={'contain'} style={{ height: Matrics.ScaleValue(20), tintColor: 'white', width: Matrics.ScaleValue(20) }} />
                                    </TouchableOpacity>
                                </View>
                            </ImageBackground>
                            :
                            <View style={{ height: Matrics.ScaleValue(180), backgroundColor: '#0073E6', width: '100%' }}>
                                <TouchableOpacity onPress={() =>this._traderprofilebackpress()}>
                                    <Image source={IMAGEPATH.BACK_WHITE} style={{ height: Matrics.ScaleValue(20), margin: Matrics.ScaleValue(30), marginTop: Matrics.ScaleValue(50), width: Matrics.ScaleValue(20) }} />
                                </TouchableOpacity>
                            </View>
                        }
                        <View style={style.logoContainer}>
                            {image ?
                                ...
                                :
                                <View style={[style.profileImageStyle, { backgroundColor: COLORS.BACKGROUND, justifyContent: 'center', alignItems: 'center' , position: 'absolute', top: Matrics.ScaleValue(-100)}]}>
                                    {/* <Text style={{ textAlign:"center", color: 'white', fontSize: Matrics.ScaleValue(25), fontWeight: '600' }}>{lastname?(firstname && firstname.charAt(0).toUpperCase()) (lastname && ' '+lastname.charAt(0).toUpperCase()):(firstname && firstname.charAt(0).toUpperCase())}</Text> */}
                                    <Text style={{ textAlign:"center", color: 'white', fontSize: Matrics.ScaleValue(25), fontWeight: '600' }}>{business_name?(business_name && business_name.charAt(0).toUpperCase()):''}</Text>
                        ...
                        <View style={{ justifyContent: 'center', alignItems: 'center', marginTop: Matrics.ScaleValue(25) }}>
                            ...
                            <TouchableOpacity onPress={() => {
                                this.setState({ renderSlide: 2,trader_id: _id, reqCategory: this.state.selectedItem.name,newCategoryId:this.state.selectedItem._id, selectedCategory: this.state.selectedItem })
                                if (this.state.loggedUserInfo != null) {
                                    console.log('this.state.loggedUserInfo',this.state.loggedUserInfo);
                                    let address = this.state.loggedUserInfo.data ?this.state.loggedUserInfo.data.address  : "";
                                    this.setState({
                                        reqName: this.state.loggedUserInfo.data.name,
                                        reqEmail: (this.state.loggedUserInfo.data.email).toLowerCase(),
...
const style = StyleSheet.create({
    ...
    backIcon:
    {
        paddingTop:Platform.OS === 'ios' ? 15:0,height:Platform.OS === 'ios' ? 40 :25,width:25,
left:0,top:0,position:'absolute',margin:Platform.OS === 'ios' ? 20: 10,zIndex:1,   },
    traderSubContainer: {
