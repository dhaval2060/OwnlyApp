...
import { showLoading, resetState } from "./ProfileAction";
...
class ProfileScreen extends Component {
  ...
  componentDidUpdate(prevProps) {
    this.onGetProfileSuccess(prevProps);
  ...
  onGetAgencyPropertySuccess() {
    if (this.props.profileReducer.agencyPropertyRes != "") {
      if (this.props.profileReducer.agencyPropertyRes.code == 200) {
        ...
        this.setState({
          propertyListData: this.props.profileReducer.agencyPropertyRes.data
        });
      } else {
        ...
        AsyncStorage.getItem("SyncittUserInfo")
          .then(value => {
            if (value) {
              ...
              console.log(postBody, "postBody", response)
              APICaller(
                "updateBannerPic",
                "POST",
                authToken,
  ...
  callGetUserProfile() {
    var roleid = "";
    this.setState({ isGetProfile: true, profileData: {} })
  ...
  onUploadBannerSuccess() {
    if (this.props.profileReducer.updateBannerImgRes != "") {
      console.log(this.props.profileReducer, "this.props.profileReducer.updateBannerImgRes")
      if (this.props.profileReducer.updateBannerImgRes.code == 200) {

        var resData = this.props.profileReducer.updateBannerImgRes.data;
        bannerImage = resData.bannerImage
          ? API.USER_IMAGE_PATH + resData.bannerImage
          : "";
  ...
  onGetUserRatingSuccess() {
    if (this.props.profileReducer.userReviewRes != "") {
      if (this.props.profileReducer.userReviewRes.code == 200) {
        ...
        this.setState({
          userReviewData: this.props.profileReducer.userReviewRes
        });
      } else {
  ...
  onGetProfileSuccess(prevProps) {
    if (this.state.isGetProfile && (this.props.profileReducer.userProfileRes != prevProps.profileReducer.userProfileRes)) {
      if (this.props.profileReducer.userProfileRes.code == 200) {

        this.setState({
          profileData: this.props.profileReducer.userProfileRes.data,
          isGetProfile: false
        });
  ...
  onGetAgencyProfileSuccess() {
    if (this.props.profileReducer.agencyProfileRes != "") {
      if (this.props.profileReducer.agencyProfileRes.code == 200) {
        ...
        this.setState({ infoData: this.props.profileReducer.userProfileRes });
      } else {
  ...
  renderImageItem(item, index) {
    return (
      <Image
        source={{ uri: item.url }}
        style={ProfileScreenStyle.userListImageStyle}
      />
    );
  }
  ...
  renderItem({ item, index }) {
    var propertyImagePath = item.image
      ? item.image.length > 0
        ? API.PROPERTY_IMAGE_PATH + item.image[0].path
        : ""
      : "";
    var userImage = item.owned_by
      ? item.owned_by.image
        ? API.USER_IMAGE_PATH + item.owned_by.image
        : ""
      : "";
    var number_of_bedroom = item.number_bedroom ? item.number_bedroom : 0;
    var number_of_bathroom = item.number_of_bathroom
    ...
    return (
      <View style={ProfileScreenStyle.listMainContainerStyle}>
        <TouchableOpacity
          onPress={ref.callPropertyDetailsScreen.bind(ref, item._id)}
        >
          <View style={ProfileScreenStyle.propertyImageViewStyle}>
            {propertyImagePath != "" ? (
              <Image
                source={{ uri: propertyImagePath }}
                style={ProfileScreenStyle.propertyImageStyle}
              />
            ) : (
            ...
            <Image
              ...
              style={ProfileScreenStyle.likeImageViewStyle}
            />
          </View>
          <View style={ProfileScreenStyle.propertyTitleViewStyle}>
            <Text
              numberOfLines={2}
              style={ProfileScreenStyle.propertyTitleTextStyle}
            >
              {item.address}
            </Text>
          </View>
          <View style={ProfileScreenStyle.propetySubTitleViewStyle}>
            <Text
              numberOfLines={2}
              style={ProfileScreenStyle.propertySubTitleTextStyle}
            >
              {item.description}
            </Text>
          </View>
          ...
          <View>
            {userImage != "" ? (
              <Image
                ...
                style={ProfileScreenStyle.userImageStyle}
              />
            ) : (
                <Image
        ...
        <View style={ProfileScreenStyle.propertyInfoContainerViewStyle}>
          <View style={ProfileScreenStyle.propertyBedroomViewContainer}>
            ...
            <Text style={ProfileScreenStyle.propertyValueTextStyle}>
              {number_of_bedroom}
            </Text>
          </View>
  ...
  navBar() {
    return (
      <View style={ProfileScreenStyle.profileHeaderContainer}>
        ...
        <TouchableOpacity
          style={ProfileScreenStyle.roundedBlueButtonStyle}
  ...
  render() {
    ...
    var userImagePath = this.state.profileData.image
      ...
      : "";
    // if (bannerImage == "") {
    let bannerImage = this.state.profileData.bannerImage
    ...
    var lastName = this.state.profileData.lastname
      ...
      : "";
    var averagerate = this.state.userReviewData
    ...
    return (
      ...
      >
        <View style={ProfileScreenStyle.profileContainer}>
          <TouchableOpacity style={ProfileScreenStyle.topCoverImageContainer}>
            {this.state.profileData.bannerImage ? (
              <Image
                source={{ uri: API.USER_IMAGE_PATH + this.state.profileData.bannerImage }}
                style={ProfileScreenStyle.topCoverImageContainer}
              />
            ) : (
          ...
          <View style={ProfileScreenStyle.bottomCoverImageContainer}>
            <Image
              source={ImagePath.PROFILE_BG}
              style={ProfileScreenStyle.bottomCoverImageContainer}
            />
          </View>
          ...
          <View style={ProfileScreenStyle.profileDataContainer}>
            <View style={ProfileScreenStyle.profileNameAndReviewContainer}>
              {userImagePath != "" ? (
                <Image
                  ...
                  style={ProfileScreenStyle.profileImageStyle}
                />
              ) : (
              ...
              <Text style={ProfileScreenStyle.userNameTextStyle}>
                {firstName + " " + lastName}
              </Text>
              <StarRating
                disabled={true}
                ...
                selectedStar={rating => ref.onStarRatingPress(rating)}
              />
              <Text style={ProfileScreenStyle.userReviewtextStyle}>
                {averagerate +
                  " " +
                  "from" +
                  " " +
              ...
              {this.state.roleName == Strings.USER_ROLE_AGENCY_OWNER ||
                this.state.roleName == Strings.USER_ROLE_STRATA_PRINCIPLE ? (
                  <TouchableOpacity
                    style={ProfileScreenStyle.contactAgentView}
                    onPress={this.callAddAgentScreen.bind(this)}
                  >
                    <View>
                      <Text style={ProfileScreenStyle.buttonTextStyle}>
                        {Strings.ADD_MANAGER}
                      </Text>
                    </View>
                  </TouchableOpacity>
            ...
            <View style={ProfileScreenStyle.userInfoContainerStyle}>
              ...
              <View style={ProfileScreenStyle.userInfoTextContainerStyle}>
                <Text style={ProfileScreenStyle.labelTextStyle}>
                  Phone number
                </Text>
                <Text style={ProfileScreenStyle.infoTextStyle}>
          ...
          <ScrollView
            ...
            contentContainerStyle={
              ProfileScreenStyle.tabContainerScrollViewStyle
            }
          >
            <View style={ProfileScreenStyle.tabContainerStyle}>
              <TouchableOpacity onPress={() => this.onAllTabClick()}>
                <View>
                    ...
                    <Text
                      style={
                        this.state.isTabSelected == 1
                          ? ProfileScreenStyle.tabLabelTextStyle
                          : ProfileScreenStyle.tabLabelDiselectTextStyle
                      }
                    >
                  ...
                  {this.state.isTabSelected == 1 ? (
                    <View style={ProfileScreenStyle.tabIndicatorStyle} />
                  ) : (
                      <View style={ProfileScreenStyle.tabWhiteIndicatorStyle} />
                    )}
                </View>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                onPress={() => this.onRequestedByTenentTabClick()}
              >
                <View>
                  <View style={ProfileScreenStyle.tabTextViewStyle}>
                    <Text
          ...
          {this.state.isTabSelected == 3 ? (
            this.state.roleName == Strings.USER_ROLE_TRADER ? (
              <TradersReviewAndRatingScreen
                reviewToId={this.state.profileData._id}
                reviewToName={
                  this.state.profileData.firstname +
                  " " +
                  this.state.profileData.lastname
        ...
        {//23 Nov
          this.props.profileReducer.isScreenLoading || this.state.isImageUploading || this.state.isGetProfile ? (
