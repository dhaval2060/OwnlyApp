    ...
    onGetAgreementDetailSuccess() {
        ...
        if (this.props.agreementDetailsReducer.agreementDetailRes != '') {
            ...
            if (this.props.agreementDetailsReducer.agreementDetailRes.code == 200) {
                console.log('agreementDetailsReducer',this.props.agreementDetailsReducer.agreementDetailRes.data);
    ...
    navBar() {
        return (
            <View style={AgreementDetailsScreenStyle.profileHeaderContainer}>
                ...
                <TouchableOpacity onPress={this.showPopup.bind(this)} style={{ marginRight: 20, marginTop: 30 }}>
            ...
            <View style={AgreementDetailsScreenStyle.profileContainer}>
                <View>
                    ...
                    <View style={{ position: 'absolute', bottom: 20, left: 20, right: 20, backgroundColor: Colors.TRANSPARENT }}>
                        <Text numberOfLines={2} style={{ color: Colors.WHITE, fontSize: 24, fontWeight: '600' }}>{PropertyData.property_address?PropertyData.property_address:''}</Text>
                        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10 }}>           
                            <View style={{ flexDirection: 'row', justifyContent: 'center' }}>
                                 <Image source={ImagePath.CALENDAR_ICON} style={{justifyContent: 'center' }} />
                                <Text style={{ color: Colors.WHITE, fontSize: 14, marginLeft: 5 }}>{Moment(PropertyData.createdAt?PropertyData.createdAt:'').format(Strings.DATE_FORMATE)}</Text>
                            </View>
                        </View>
                    </View>
                    {this.navBar()}
