...
import { showLoading, resetState } from "../AgentsComponent/AgentsScreenAction";
let ref;
class OverviewScreen extends Component {
  ...
  onAgentListWithiInAgencySuccess() {
    if (this.props.agentsScreenReducer.agentListWithInAgencyResponse != "") {
      if (
        ...
        this.setState({
          agentListData: this.props.agentsScreenReducer
            .agentListWithInAgencyResponse.data
        });
      } else {
      }
      this.props.resetState();
  ...
  uploadImageListSelection(index) {
    var overviewImagePath = this.state.overViewData.images
      ? this.state.overViewData.images.length > 0
        ? this.state.overViewData.images[index].url
        : ""
      : "";
      ...
      >
        <View style={OverviewScreenStyle.overviewImageListItemStyle}>
          <Image
            ...
            style={OverviewScreenStyle.overviewPropertyListImageStyle}
          />
        </View>
        {item.isSelected == 1 ? (
          <View style={OverviewScreenStyle.selectedImageStyle} />
        ) : null}
      </TouchableOpacity>
    );
  }
  ...
  agentRenderItem({ item, index }) {
    ...
    return (
      <View style={OverviewScreenStyle.listItemContainer}>
          ...
          >
            {userImage != "" ? (
              <Image
                ...
                style={OverviewScreenStyle.listImageStyle}
              />
            ) : (
                <View style={OverviewScreenStyle.emptyMaintenaceUserImageStyle}>
                  <Text style={OverviewScreenStyle.initialTextStyle}>
        ...
        <Text style={OverviewScreenStyle.listTitleTextStyle}>
          {firstName + " " + lastName}
        </Text>
        <View>
          <StarRating
        ...
        <Text style={OverviewScreenStyle.listReviewTextStyle}>
          {averageRate +
            " " +
            "from" +
            " " +
  ...
  render() {
    var total_manager = this.state.infoData
      ? this.state.infoData.total_manager
      : 0;
    var total_property = this.state.infoData
      ? this.state.infoData.data.length > 0
        ? this.state.infoData.data[1].value
        : 0
      : 0;
    ...
    var totalreviews = this.state.userReviewData.total_review
      ...
      : 0;
    var agencyImage = this.state.overViewData.agency_id
      ? this.state.overViewData.agency_id.logoImage
    ...
    return (
      <ScrollView contentContainerStyle={{ paddingBottom: 70 }}>
        {this.state.selectedImage != "" ? (
          <Image
            source={{ uri: API.USER_IMAGE_PATH + this.state.selectedImage }}
            style={OverviewScreenStyle.overviewImageStyle}
          />
        ) : null}
        ...
        <View>
          <Text style={OverviewScreenStyle.titleTextStyle}>
            About{" "}
            {this.state.overViewData.firstname +
              " " +
              this.state.overViewData.lastname ? this.state.overViewData.lastname : ""}
          </Text>
          <Text style={OverviewScreenStyle.detailsTextStyle}>
            {this.state.overViewData.about_user
              ? this.state.overViewData.groups.about_user
              : "No data found"}
          </Text>
        </View>
        ...
        <View>
          <View style={OverviewScreenStyle.userInfoTextContainerStyle}>
            <Text style={OverviewScreenStyle.labelTextStyle}>
              Properties being managed
            </Text>
            <Text style={OverviewScreenStyle.valueTextStyle}>
              {total_property}
            </Text>
          </View>
