...
import ApplySyncProfileScreenStyle from "./OccupancyScreenStyle";
...
let contextRef;
class OccupancyScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      uploadImagesData: {},
                  ...
                  data => {
                    ...
                    if (data.code == 200) {
                      this.setState({
                        ...
                        personCount: data.data.members ? data.data.members.length : 0,
                        PetCount: data.data.pets ? data.data.pets.length : 0,
                        vehicleCount: data.data.vehicles ? data.data.vehicles.length : 0
                      });
                    }
                  }
                );
          ...
          AsyncStorage.getItem("SyncittUserInfo")
            .then(value => {
              if (value) {
                ...
                APICaller("updateOccupacy", "POST", authToken, postData).then(
                  data => {
  ...
  setAge(val, index) {
    ...
    arr[index]["age"] = val;
    this.setState({ members: arr, errorMsg: "", showAgeDropDrown: "" });
  }
      ...
      <View style={{ flex: 1 }}>
            ...
            >
              ...
              <View style={ApplySyncProfileScreenStyle.addPropertyInputContainer}>
                ...
                >
                  Are You confirm that you have the person's content to provide
...
function mapStateToProps(state) {
  return {
      ...
      export default OccupancyScreen;
