...
class TradersMaintenanceRequestScreen extends Component {
            ...
            if (value) {
                ...
                AsyncStorage.getItem("roleId").then((role) => {
                    if (role) {
                        ...
                        console.log("here " + JSON.stringify(postData) + " l " + JSON.stringify(authToken))
                        // {"request_by_role":"5a1d26b26ef60c3d44e9b377","request_by_id":"5e69dceab115fb15d000b40c","public_status":"yes"}                        
                        APICaller('maintenanceList', 'POST', authToken, postData).then(data => {
                            ...
                            if (data.code == 200) {
                                ...
                                this.setState({ publicMaintenance: maintainList11, loading: false, publicMaintenanceBackUp: maintainList11 }
                                    //     ,()=>
                                    // alert(this.state.publicMaintenance.length)
                                )
                            }else{
                                this.setState({ publicMaintenance: [], loading: false, publicMaintenanceBackUp: [] })
                            }
                        })
    ...
    onPublicTabClick(val) {
        ...
        if (!val) {
            this.setState({ isTabSelected: 5, loading: true }
                , () =>
                    this.callPublic(!val)
            )
        ...
        return (
            ...
            <View style={CommonStyles.listMainContainerStyle}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={CommonStyles.flatListStyle}>
                        ...
                        this.state.newData ?
                            (this.state.isTabSelected == 2) ?
                                this.state.newData.length === 0 ?
                                    <View style={{ justifyContent: 'center', position: 'absolute', alignItems: 'center', alignSelf: 'center', }}>
                                        <ActivityIndicator size='large' color={Colors.SKY_BLUE_BUTTON_BACKGROUND} />
                                    </View>
                                    :
                    ...
                    {this.state.completedData ?
                        (this.state.isTabSelected == 5) ?
                            this.state.publicMaintenance.length === 0&& this.state.loading ?
                                <TouchableOpacity style={{ justifyContent: 'center', position: 'absolute', alignItems: 'center', alignSelf: 'center', }}>
                                    <ActivityIndicator size='large' color={Colors.SKY_BLUE_BUTTON_BACKGROUND} />
                                </TouchableOpacity>
                                    ...
                                    ListEmptyComponent={()=>{
                                        return(
                                            <View style={{height:150,justifyContent:'center',alignItems:'center'}}>
                                                <Text>No public request found</Text>
                                            </View>
                                        )
                                    }}
                                    extraData={this.state}
