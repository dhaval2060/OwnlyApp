...
class TradersProfileScreen extends Component {
    ...
    callGetAgencyProfile() {
        ...
        AsyncStorage.getItem("SyncittUserInfo").then((value) => {
            if (value) {
                ...
                if(this.state.profileData.agency_id){
    ...
    callGetUserRating() {
        ...
        AsyncStorage.getItem("SyncittUserInfo").then((value) => {
            if (value) {
                if(this.state.profileData.agency_id && this.state.profileData.agency_id.principle_id){
                    var userData = JSON.parse(value);
                    var authToken = userData.token;
                    this.props.showLoading();
        ...
        if (this.props.tradersReducer.tradersProfileRes != '') {
            if (this.props.tradersReducer.tradersProfileRes.code == 200) {
                this.setState({ isScreenLoading: false, profileData: this.props.tradersReducer.tradersProfileRes.data });
                    console.log(this.props.tradersReducer.tradersProfileRes.data);
                    ...
                    <View style={AgentProfileScreenStyle.profileContainer}>
                        ...
                        <View style={AgentProfileScreenStyle.profileDataContainer}>
                            <View style={AgentProfileScreenStyle.profileNameAndReviewContainer}>
                                ...
                                <Text style={[AgentProfileScreenStyle.userNameTextStyle,{fontSize:20,fontWeight:'500',letterSpacing:0.5}]}>{agentName}</Text>
                                <StarRating
                                    disabled={true}
                                    maxStars={5}
                                    starSize={20}
                                ...
                                />
                                <Text style={[AgentProfileScreenStyle.userReviewtextStyle,{fontSize:16}]}>{averageRate + ' ' + 'from' + ' ' + totalreviews + ' ' + 'reviews'}</Text>
                            ...
                            <View style={AgentProfileScreenStyle.userInfoContainerStyle}>
                                <View style={AgentProfileScreenStyle.userInfoTextContainerStyle}>
                                    ...
                                    <Text style={AgentProfileScreenStyle.infoTextStyle}>{this.state.profileData.address?this.state.profileData.address:'-'}</Text>
                                </View>
                                <View style={AgentProfileScreenStyle.userInfoTextContainerStyle}>
                                    <Text style={AgentProfileScreenStyle.labelTextStyle}>Phone number</Text>
                                    <View style={{flexDirection:'row',justifyContent:'center'}}>
                                        <Text style={AgentProfileScreenStyle.infoTextStyle}>{this.state.mobileVisible?this.state.profileData.mobile_no:lastFourmobileNo}</Text>
                                        {
                                        !this.state.mobileVisible && (
                                            <TouchableOpacity onPress={()=>this._mobileNoCountAPI()}>
                                                <Text style={[AgentProfileScreenStyle.infoTextStyle,{color:Colors.GRAY,fontSize:10,marginLeft:5}]}>{'SHOW NUMBER'}</Text>
                                            </TouchableOpacity>    
                                        )
                                    }
                                    </View>
