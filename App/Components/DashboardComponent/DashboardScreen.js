...
class Dashboard extends Component {
	...
	componentWillMount() {
		...
		this._getNotificationCounter();
	}

	_getNotificationCounter = () => {
		let counter = 0;
		AsyncStorage.getItem("SyncittUserInfo").then(async (value) => {
			if (value) {
				...
				await APICaller("notificationList", "POST", authToken, params).then(async (data) => {
					if(data.code === 200){
						data.data.map((item) => {
						...
						console.log('Notificationcounter',counter);
	...
	componentDidUpdate() {
		...
		if (this.props.switchProfileReducer.switchUserProfileRes != '') {
			if(this.props.switchProfileReducer.switchUserProfileRes.data !== ''){
	...
	callGetUserRole() {
		AsyncStorage.getItem("SyncittUserInfo").then((value) => {
			if (value) {
	// Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVlNjlkYWZjYjExNWZiMTVkMDAwYjQwOCIsImlhdCI6MTU5NTU5MDI5MywiZXhwIjoxNTk2MTk1MDkzfQ.YxhJ1jGGm93kkLbTN5jb1ISZj0gzi3t_5rW2RU0xpXg"
   //  Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVlNjlkYWZjYjExNWZiMTVkMDAwYjQwOCIsImlhdCI6MTU5NTU5MDI5MywiZXhwIjoxNTk2MTk1MDkzfQ.YxhJ1jGGm93kkLbTN5jb1ISZj0gzi3t_5rW2RU0xpXg
	// Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVlNjlkYWZjYjExNWZiMTVkMDAwYjQwOCIsImlhdCI6MTU5NjY1MzY2MywiZXhwIjoxNTk3MjU4NDYzfQ.9dmQfJYtGsgE2aylPXUgzfzD5L44FWNmgoudD8tom8I			
   var userData = JSON.parse(value);
// console.log("tokennnnnnnnnnn"+JSON.stringify(userData.token))
				...
				<View style={DashboardStyle.drawerContentView} >
						...
						<View>
							{
								...
								<View style={{position:'absolute',backgroundColor:'red',bottom:0,right:5,borderRadius:5,padding:2,zIndex:100}}>
									<Text style={{color:'white',fontSize:10,fontWeight:'bold'}}>{global.notificationcounter}</Text>
								</View>
							}
							<TouchableOpacity onPress={() => this.callNotificationScreen()} style={{alignSelf:'flex-start'}}>
								<View>
									<Image source={ImagePath.DRAWER_NOTIFICATION_ICON_DASHBOARD} />
								</View>
							</TouchableOpacity>
						</View>
						<View style={{flexDirection:'row',alignItems:'center',justifyContent:'space-between'}}>
						<TouchableOpacity onPress={() => this.callSearchScreen()}>
								<Image source={ImagePath.DRAWER_SEARCH_ICON_DASHBOARD} style={DashboardStyle.searchImageStyle} />
							</TouchableOpacity>
							<TouchableOpacity onPress={() => this.closeControlPanel()}>
								<Image source={ImagePath.DRAWER_CROSS_ICON_DASHBOARD} />
							</TouchableOpacity>
						</View>
					</View>
					...
					{/* <View style={DashboardStyle.headerViewStyle}> */}
						{/* <View style={{flex:1}}>
							{
								global.notificationcounter !== 0 &&
								<View style={{position:'absolute',backgroundColor:'red',bottom:-8,left:4,borderRadius:5,padding:2}}>
									<Text style={{color:'white',fontSize:10,fontWeight:'bold'}}>{global.notificationcounter}</Text>
								</View>
							}
							<TouchableOpacity onPress={() => this.callNotificationScreen()} style={{alignSelf:'flex-start'}}>
			...
			<View style={{ flex: 1 }}>
				<Drawer
					...
					<BottomNavigation
						...
						innerStyle={{ paddingTop: 5,paddingBottom:10 }}
						onTabChange={this.handleTabChange}
						shifting={false}>

						<Tab
							barBackgroundColor="#FFFFFF"
							label={<Text style={{fontSize:12,alignSelf:'center',padding:5}}>{`Home`}</Text>}
							icon={<Image source={ImagePath.DASHBOARD_HOME_ACTIVE} resizeMode={'contain'} style={[CommonStyles.bottomTabIcons1, { tintColor: 'gray' }]} name="Home"/>}
							activeIcon={<Image source={ImagePath.DASHBOARD_HOME_ACTIVE} resizeMode={'contain'} style={CommonStyles.bottomTabIcons1} />}
						/>
						<Tab
							barBackgroundColor="#FFFFFF"
							label={<Text style={{fontSize:12,alignSelf:'center',padding:5}}>{`Properties`}</Text>}
							icon={<Image source={ImagePath.DASHBOARD_PROPERTY_ACTIVE_NEW} resizeMode={'contain'} style={[CommonStyles.bottomTabIcons1, { tintColor: 'gray' }]} />}
							activeIcon={<Image source={ImagePath.DASHBOARD_PROPERTY_ACTIVE_NEW} resizeMode={'contain'} style={CommonStyles.bottomTabIcons1} />}
						/>
						<Tab
							barBackgroundColor="#FFFFFF"
							label={<Text style={{fontSize:12,alignSelf:'center',padding:5}}>{`Messages`}</Text>}
							icon={
								<View style={{ flex: 1 }}>
									{global.unRead != 0 &&
										<View style={{
						...
						<Tab
							barBackgroundColor="#FFFFFF"
							label={<Text style={{fontSize:12,alignSelf:'center',padding:5}}>{`Maintenance`}</Text>}
							icon={<Image source={ImagePath.DASHBOARD_REQUEST_ACTIVE_NEW} resizeMode={'contain'} style={[CommonStyles.bottomTabIcons1, { tintColor: 'gray' }]} />}
							activeIcon={<Image source={ImagePath.DASHBOARD_REQUEST_ACTIVE_NEW} resizeMode={'contain'} style={CommonStyles.bottomTabIcons1} />}
						/>
						<Tab
							barBackgroundColor="#FFFFFF"
							label={<Text style={{fontSize:12,alignSelf:'center',padding:5}}>{`More`}</Text>}
							icon={
								<View style={{ flex: 1 }}>
									{global.notificationcounter != 0 &&
										<View style={{
											...
											shadowOffset: { height: 0, width: 0 },
											justifyContent: 'center',
											shadowColor: 'grey', zIndex: 9, left: 20, height: 12, top: -2, width: 12, borderRadius: 6, backgroundColor: 'red'
										}}>
											{/* <Text style={{ fontSize: 8, color: 'red', backgroundColor: 'transparent', textAlign: 'center' }}>{global.unRead}</Text> */}
										</View>
									}
									<Image source={ImagePath.DASHBOARD_MENU_ACTIVE} resizeMode={'contain'} style={[{ flex: 1, tintColor: 'gray', position: "absolute", top: 0, zIndex: 0 }, CommonStyles.bottomTabIcons1]} />
								</View>
							}
							activeIcon={
								<View style={{ flex: 1 }}>
									...
									<Image source={ImagePath.DASHBOARD_MENU_ACTIVE} resizeMode={'contain'} style={[{ flex: 1, position: "absolute", top: 0, zIndex: 0 }, CommonStyles.bottomTabIcons1]} />
								</View>
							}
							// icon={<Image source={ImagePath.DASHBOARD_MENU_ACTIVE} resizeMode={'contain'} style={[CommonStyles.bottomTabIcons, { tintColor: 'gray',marginBottom:4 }]} />}
							// activeIcon={<Image source={ImagePath.DASHBOARD_MENU_ACTIVE} resizeMode={'contain'} style={[CommonStyles.bottomTabIcons,{marginBottom:5}]} />}
						/>
				...
				{/* <Modal visible={!this.state.isUserSubscribe} transparent={true} onRequestClose={() => { }}>
					<View style={{ backgroundColor: Colors.TRANSLUCENT_BLACK_DARK, justifyContent: 'center', alignItems: 'center', flex: 1, width: '100%', height: '100%' }}>
						<View style={{ height: '30%', width: '90%', backgroundColor: 'white', justifyContent: 'center', alignItems: 'center', borderRadius: 10 }}>
							...
							<Text style={{ textAlign: 'center', fontSize: 18, color: Colors.GRAY_COLOR, fontWeight: '600' }}>Please Activate your Account by Selecting a Plan</Text>
							<TouchableOpacity onPress={() => {
								this.setState({ activeTab: 4 })
								this.onSetting()
								this.setState({ isUserSubscribe: true, isClickOnModal: true })
							}} style={{ margin: 10, marginTop: 30, justifyContent: 'center', alignItems: 'center', height: 50, width: 150, backgroundColor: Colors.SKY_BLUE_BUTTON_BACKGROUND, borderRadius: 10 }}>
								<Text style={{ fontSize: 20, fontWeight: '600', color: Colors.WHITE }}>Okay</Text>
							</TouchableOpacity>
						</View>
					</View>
				</Modal> */}
